...
def output_sequences(oneword = False):
    lex = get_lexicon('/Users/Marina/data/bias-lexicon/bias-lexicon.txt')
    ...
    for dset in ['test', 'train', 'dev']:
        df_class, df_tag, df_ow_tag = filter_edits('/Users/Marina/data/npov-edits/5gram-edits-%s.tsv' %dset, lex)
        #write_sequences('/Users/Marina/data/npov-edits/%s/data.txt'%dset, seqs)
        #pickle.dump(df_class, open(f'/Users/Marina/data/npov-edits/Classification_%s.pkl'%dset, 'wb'))
        pickle.dump(df_tag, open(f'/Users/Marina/data/npov-edits/Tagging_%s.pkl' % dset, 'wb'))
...
#lex = get_lexicon('/Users/Marina/data/bias-lexicon/bias-lexicon.txt')
#dset = 'test'
#df = filter_edits('/Users/Marina/data/npov-edits/5gram-edits-%s.tsv' %dset, lex)
#print( df)

def get_bias_set(file):
    hedges = set()
    with open("/Users/Marina/Downloads/bias_related_lexicons/%s" % file, 'r') as hedgefile:
        try:
            for l in hedgefile:
...
def parse_wiebe():
    ...
    with open("/Users/Marina/Downloads/subjectivity_clues_hltemnlp05/subjclueslen1-HLTEMNLP05.tff",
              'r') as hedgefile:
        for l in hedgefile:
...
def add_features():
    ...
    for dset in ['test', 'train', 'dev']:
        ...
        srcfile = open("/users/Marina/OpenNMT/data/npov/src-%s-seq-feat-data.txt"%dset, "w")
        tgtfile = open("/users/Marina/OpenNMT/data/npov/tgt-%s-seq-feat-data.txt"%dset, "w")

        srcfile_ow = open("/users/Marina/OpenNMT/data/npov/src-%s-seq-feat-data_ow.txt"%dset, "w")
        tgtfile_ow = open("/users/Marina/OpenNMT/data/npov/tgt-%s-seq-feat-data_ow.txt"%dset, "w")
...
def write_featured_seq(df):
    srcfile = open("/users/Marina/data/npov-edits/src-test-seq-feat-data.txt", "w")
    tgtfile = open("/users/Marina/data/npov-edits/tgt-train-seq-feat-data.txt", "w")
